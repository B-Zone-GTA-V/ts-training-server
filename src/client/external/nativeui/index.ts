...
export default class NativeUI {
	...
	public IsMouseInListItemArrows(
		...
		let res = UiScreen.ResolutionMaintainRatio();
		...
		return UiScreen.IsMouseInBounds(topLeft, new Size(labelSizeX, 38))
			? 1
			: UiScreen.IsMouseInBounds(
	...
	public ProcessMouse() {
		...
		if (
			UiScreen.IsMouseInBounds(new Point(0, 0), new Size(30, 1080)) &&
			...
			UiScreen.IsMouseInBounds(
				new Point(UiScreen.ResolutionMaintainRatio().Width - 30.0, 0),
				new Size(30, 1080)
			) &&
			this.MouseEdgeEnabled
		) {
		...
		for (let i = this._minItem; i <= limit; i++) {
			...
			if (UiScreen.IsMouseInBounds(new Point(xpos, ypos), new Size(xsize, ysize))) {
	...
	public CalculateDescription() {
		if(this.MenuItems.length > 0) {
			...
			if(this._descriptionText.caption && this.MenuItems[this._activeItem % this.MenuItems.length].Description.trim() !== "") {
				const numLines = UiScreen.GetLineCount(this._descriptionText.caption, this._descriptionText.pos, this._descriptionText.font, this._descriptionText.scale, this._descriptionText.Wrap, ResText);
