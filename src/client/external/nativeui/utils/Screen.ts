...
export const UiScreen = {
	...
	getMousePosition: (relative: boolean = false) => {
		const res = UiScreen.ResolutionMaintainRatio();
	...
	IsMouseInBounds: (topLeft: Point, boxSize: Size) => {
		const [mouseX, mouseY] = UiScreen.getMousePosition();
		return (
			mouseX >= topLeft.X &&
	...
	GetTextWidth: (text: string, font: Font, scale: number, resText: any) => {
	...
	GetLineCount: (text: string, position: Point, font: Font, scale: number, wrap: number, resText: any) => {
		...
		const res: Size = UiScreen.ResolutionMaintainRatio();
